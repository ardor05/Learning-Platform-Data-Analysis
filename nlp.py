...
def app():
    # Load the dataset
    file_path = r'C:/Users/ACER/Downloads/capstone/cleaned_logs_NLP1.0.csv'
    ...
    # Re-map cluster labels based on mean activity count
    cluster_means = daily_activity.groupby('cluster')['activity_count'].mean().sort_values()
    cluster_mapping = {old_label: new_label for new_label, old_label in enumerate(cluster_means.index)}
    daily_activity['cluster'] = daily_activity['cluster'].map(cluster_mapping)

    # Define a custom palette for clusters
    custom_palette = {0: 'green', 1: 'purple', 2: 'yellow'}
    ...
    **Cluster Distribution:**
    ...
    - **Cluster 0 (Teal):** Represents days with relatively low activity counts. These dots are mostly concentrated near the bottom of the y-axis, indicating low activity.
    - **Cluster 1 (Purple):** Represents days with moderate activity counts. These dots are spread out more along the y-axis, indicating a moderate level of student activity.
    - **Cluster 2 (Yellow):** Represents days with high activity counts. These dots are mostly found higher up on the y-axis, indicating higher activity levels.

  
    """)
    fig, ax = plt.subplots(figsize=(12, 6))
    sns.scatterplot(data=daily_activity, x='date', y='activity_count', hue='cluster', palette=custom_palette, ax=ax)
